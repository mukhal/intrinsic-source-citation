...
class PrefixConstrainedLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, prefix_allowed_tokens_fn: Callable[[int, torch.Tensor], List[int]], num_beams: int, 
        ...
        self._decoding_trie = decoding_trie
        self._prefix_allowed_tokens_fn = lambda _, x: (self._decoding_trie.get(x.tolist()) if x[-1] not in [1 , 0, 2] else [x[-1]])

    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        mask = torch.full_like(scores, -math.inf)
        for batch_id, beam_sent in enumerate(input_ids.view(-1, self._num_beams, input_ids.shape[-1])):
            for beam_id, sent in enumerate(beam_sent):
                inp_dim = self._input_shape[1] if self._input_shape is not None else 1
                #print("sent: ", sent)
                #next = self._prefix_allowed_tokens_fn(batch_id, sent[inp_dim - 1:])
                #print('Constrained: ', self._prefix_allowed_tokens_fn(batch_id, sent[inp_dim - 1:]))
                mask[batch_id * self._num_beams + beam_id, self._prefix_allowed_tokens_fn(batch_id, sent[inp_dim - 1:])] = 0
