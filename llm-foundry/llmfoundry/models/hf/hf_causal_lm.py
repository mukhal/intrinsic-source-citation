...
from transformers.models.gpt2.modeling_gpt2_2d_attention import GPT2LMHeadModelWith2DAttention
from transformers.models.llama.modeling_llama_2d_attention import LlamaForCausalLMWith2DAttention
        ...
        if isinstance(om_model_config, DictConfig):
            ...
            if resolved_init_device == 'cpu':
                if om_model_config.pretrained:
                    if 'gpt' in om_model_config.pretrained_model_name_or_path:
                        print('Loading GPT2LMHeadModelWith2DAttention...')
                        model = GPT2LMHeadModelWith2DAttention.from_pretrained(
                            om_model_config.pretrained_model_name_or_path,
                            use_auth_token=use_auth_token,
                            config=config)
                    elif 'llama' in om_model_config.pretrained_model_name_or_path or 'Llama' in om_model_config.pretrained_model_name_or_path:
                        print('Loading LlamaForCausalLMWith2DAttention...')
                        model = LlamaForCausalLMWith2DAttention.from_pretrained(
                            om_model_config.pretrained_model_name_or_path,
                            use_auth_token=use_auth_token,
                            config=config)
                    else:
                        raise ValueError("Model {} not supported".format(om_model_config.pretrained_model_name_or_path))
                else:
                    warnings.warn("Loading model from scratch!!")
            ...
            loss_args = om_model_config.get('loss', {})
